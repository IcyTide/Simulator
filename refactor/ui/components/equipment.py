...
def build_equipment():
            ...
            for label, equip in POSITION_TRANSLATE.items():
                ...
                with gr.Tab(label=label):
                    with gr.Row():
                        with gr.Column(scale=7):
                            with gr.Row():
                                equipment_dropdown = gr.Dropdown(choices=[""] + list(sub_equipments.index), label="装备",
                                                                 value="", scale=6)
                                strength_level = gr.Dropdown(choices=list(range(MAX_STRENGTH_LEVEL + 1)), label="精炼",
                                                             value=MAX_STRENGTH_LEVEL, visible=False, scale=4)
                ...
                def dropdown_func(position, equipment_map, enchant_map):
                    def inner(equip_name, enchant_name, stone_name, equip_attr):
                        ...
                        if stone_name:
                            ...
                            equip_attr["stone"] = get_stone_attr(stone_row)
                        else:
                            equip_attr["stone"] = {}
                ...
                stone_level.change(stone_level_func, stone_level, stone_dropdown)
                ...
                def equip_attr_func(name):
                    def inner(equip_attr, equips_attr):
                        ...
                        equips_attr[name] = {"attrs": attrs, "set": [equip_attr['set_id'], equip_attr['set_data']],
                                             ...
                                             "names": [equip_attr['equip_name'], equip_attr['enchant_name'],
                                                       equip_attr['stone_name']]}
    ...
    def equips_attr_func(equips_attr):
        ...
        for name in POSITION_TRANSLATE:
            ...
            equip_text = '\t\t'.join(equip_attr['names'])
        ...
        return "\n".join([f"{ATTR_TYPE_TRANSLATE[k]}: {v}" for k, v in attrs.items() if v]), "\n".join(
            [GAINS_TRANSLATE[e] for e in gains]), "\n".join(attr_texts), "\n".join(equip_texts), attribute, [
                   GAINS_MAP[e] for e in gains]
