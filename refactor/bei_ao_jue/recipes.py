...
def lei_zou_feng_qie_5_damage(status: Status):
    status.skills["雷走风切"].damage_addition_gain += 0.05


def lei_zou_feng_qie_4_damage(status: Status):
    status.skills["雷走风切"].damage_addition_gain += 0.04


def lei_zou_feng_qie_3_damage(status: Status):
    status.skills["雷走风切"].damage_addition_gain += 0.03


def lei_zou_feng_qie_4_critical(status: Status):
    status.skills["雷走风切"].critical_strike_gain += 0.04


def lei_zou_feng_qie_3_critical(status: Status):
    status.skills["雷走风切"].critical_strike_gain += 0.03


def lei_zou_feng_qie_2_critical(status: Status):
    status.skills["雷走风切"].critical_strike_gain += 0.02


def po_fu_chen_zhou_5_damage(status: Status):
    status.skills["破釜沉舟"].damage_addition_gain += 0.05


def po_fu_chen_zhou_4_damage(status: Status):
    status.skills["破釜沉舟"].damage_addition_gain += 0.04


def po_fu_chen_zhou_3_damage(status: Status):
    status.skills["破釜沉舟"].damage_addition_gain += 0.03


def po_fu_chen_zhou_4_critical(status: Status):
    status.skills["破釜沉舟"].critical_strike_gain += 0.04


def po_fu_chen_zhou_3_critical(status: Status):
    status.skills["破釜沉舟"].critical_strike_gain += 0.03


def po_fu_chen_zhou_2_critical(status: Status):
    status.skills["破釜沉舟"].critical_strike_gain += 0.02


def shang_jiang_jun_yin_4_damage(status: Status):
    status.skills["上将军印"].damage_addition_gain += 0.04
    status.skills["见尘"].damage_addition_gain += 0.04


def shang_jiang_jun_yin_3_damage(status: Status):
    status.skills["上将军印"].damage_addition_gain += 0.03
    status.skills["见尘"].damage_addition_gain += 0.03


def shang_jiang_jun_yin_2_damage(status: Status):
    status.skills["上将军印"].damage_addition_gain += 0.02
    status.skills["见尘"].damage_addition_gain += 0.02


def shang_jiang_jun_yin_4_critical(status: Status):
    status.skills["上将军印"].critical_strike_gain += 0.04


def shang_jiang_jun_yin_3_critical(status: Status):
    status.skills["上将军印"].critical_strike_gain += 0.03


def shang_jiang_jun_yin_2_critical(status: Status):
    status.skills["上将军印"].critical_strike_gain += 0.02


def qin_long_liu_zhan_5_damage(status: Status):
    related_skills = ["惊燕式", "逐鹰式"]
    for skill in related_skills:
        status.skills[skill].damage_addition_gain += 0.05


def qin_long_liu_zhan_4_damage(status: Status):
    related_skills = ["惊燕式", "逐鹰式"]
    for skill in related_skills:
        status.skills[skill].damage_addition_gain += 0.04


def qin_long_liu_zhan_3_damage(status: Status):
    related_skills = ["惊燕式", "逐鹰式"]
    for skill in related_skills:
        status.skills[skill].damage_addition_gain += 0.03


def qin_long_liu_zhan_4_critical(status: Status):
    related_skills = ["惊燕式", "逐鹰式"]
    for skill in related_skills:
        status.skills[skill].critical_strike_gain += 0.04


def qin_long_liu_zhan_3_critical(status: Status):
    related_skills = ["惊燕式", "逐鹰式"]
    for skill in related_skills:
        status.skills[skill].critical_strike_gain += 0.03


def qin_long_liu_zhan_2_critical(status: Status):
    related_skills = ["惊燕式", "逐鹰式"]
    for skill in related_skills:
        status.skills[skill].critical_strike_gain += 0.02
...
def dao_xiao_feng_yin_4_damage(status: Status):
    status.skills["刀啸风吟"].damage_addition_gain += 0.04


def dao_xiao_feng_yin_4_critical(status: Status):
    status.skills["刀啸风吟"].critical_strike_gain += 0.04


def dao_xiao_feng_yin_3_critical(status: Status):
    status.skills["刀啸风吟"].critical_strike_gain += 0.03


def dao_xiao_feng_yin_2_critical(status: Status):
    status.skills["刀啸风吟"].critical_strike_gain += 0.02
